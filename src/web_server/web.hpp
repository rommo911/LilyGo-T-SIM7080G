...
namespace fs
{
    ...
    static const String imuThresholdsPage =
        ...
        "<input id=wom type=range min=1.0 max=30.0 step=0.1 value=15.0 />"
        "<label>WOM Sleep <span id=val_woms></span></label>"
        "<input id=woms type=range min=1.0 max=30.0 step=0.1 value=15.0 />"
        ...
        "  document.getElementById('val_woms').innerText = Number(document.getElementById('woms').value).toFixed(1);"
        "}"
        "function saveIMU() {"
        "  const payload = {"
        ...
        "    WOM_THR: parseFloat(document.getElementById('wom').value),"
        "    WOM_THR_S: parseFloat(document.getElementById('woms').value)"
        "  };"
        ...
        "    document.getElementById('wom').value = d.WOM_THR;"
        "    document.getElementById('woms').value = d.WOM_THR_S;"
        "    updateValues();"
        "  }).catch(()=>updateValues());"
        "  ['gx','gy','gz','roll','yaw','pitch','wom','woms'].forEach(id=>document.getElementById(id).addEventListener('input',updateValues));"
        "});"
        "</script>" +
        style;
